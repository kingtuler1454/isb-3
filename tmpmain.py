...
class Window(QMainWindow):
    ...
    def button_OK_click(self)->None:
        ...
        if self.chose_task==1:
            ...
            try:
                symmetric_key=Object_deserialization.deserialization_sym_key(self.path_field3)
                private_key=Object_deserialization.deserialization_private_key(self.path_field2)
                ...
                Object_chipher.encrypted_text_symmetric_algorithm( d_symmetric_key,padded_text,self.path_field4)
            except FileNotFoundError :
                QMessageBox.about(self, "Внимание", "Проверьте введённые данные")
        else:
            Object_deserialization=Deserialization( )
            symmetric_key=Object_deserialization.deserialization_sym_key(str(self.field_third.text()))
            private_key=Object_deserialization.deserialization_private_key(str(self.field_second.text()))
            public_key=private_key.public_key()
            Object_chipher=Chipher(public_key,symmetric_key,private_key)
            d_symmetric_key=Object_chipher.decrypted_symmetric_key()
    ...
    def __init__(self) -> None:
        ...
        self.path_field1=self.path_field2=self.path_field3=self.path_field4=os.getcwd()
        self.chose_task=1
        self.hide_field()
        self.set_design()
